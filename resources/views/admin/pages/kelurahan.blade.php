<x-admin-layout>
    ...
    <div class="bg-gray-100 py-4 space-y-2">
        <h1 class="py-3 text-5xl font-semibold text-gray-800">Daftar Kelurahan di
            {{ $kecamatan->nama_kecamatan }}
        </h1>
        <x-breadcrumb :breadcrumbs="[
        ['name' => 'Kelola Data', 'url' => route('data.index')],
        ['name' => 'Daftar Kelurahan', 'url' => route('kelurahan.index', ['kecamatan_id' => $kecamatan->id])],
    ]" />
    </div>
                    ...
                    <div class="bg-white shadow border border-gray-200 rounded-lg divide-y divide-gray-200">
                        <div class="py-3 px-4">
                            <div class="flex justify-between items-center">
                                <x-admin.search-input id="searchInput" placeholder="Cari kelurahan..."
                                    onkeyup="searchTable()" />
                                    ...
                                    @foreach ($kelurahans as $kelurahan)
                                        <tr>
                                            <td class="px-6 py-3 text-sm text-center font-medium text-gray-800">
                                                {{ ($kelurahans->currentPage() - 1) * $kelurahans->perPage() + $loop->iteration }}
                                            </td>
                                            <td class="px-6 py-3 text-sm font-medium text-gray-800">
                                                {{ $kelurahan->nama_kelurahan }}
                                            </td>
                                            <td class="px-6 py-3 text-sm text-center font-medium text-gray-800">
                                                {{ $kelurahan->mesjids_count }}
                                            </td>
                                            ...
                                            <td class="flex gap-x-3 justify-end px-6 py-3 text-end text-sm font-medium">
                                                <a href="{{ route('mesjid.index', ['kelurahan_id' => $kelurahan->id]) }}"
                                                ...
                                                <button type="button"
                                                    onclick="openEditModal({{ $kelurahan->id }}, '{{ $kelurahan->nama_kelurahan }}')"
                                                ...
                                                <button type="button"
                                                    onclick="openModal({{ $kelurahan->id }}, 'delete-kelurahan-modal')"
...
<!-- Tambah Modal -->
<div id="tambah-kelurahan-modal" class="fixed inset-0 z-[60] hidden">
    <div class="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
    <div class="flex min-h-full items-center justify-center">
        <div
            class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            <form id="tambah-kelurahan-form" method="POST" action="{{ route('kelurahan.store') }}">
                @csrf
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="w-full">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="tambah_nama_kelurahan"
                                        class="block text-sm font-medium text-gray-700">Nama Kelurahan</label>
                                    <input type="text" id="tambah_nama_kelurahan" name="nama_kelurahan"
...
<!-- Edit Modal -->
<div id="edit-kelurahan-modal" class="fixed inset-0 z-[60] hidden">
    <div class="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
    <div class="flex min-h-full items-center justify-center">
        <div
            class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            <form id="edit-kelurahan-form" method="POST">
                @csrf
                @method('PUT')
                <input type="hidden" id="edit_kelurahan_id" name="id">
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="w-full">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Edit Kelurahan</h3>
                            <div class="space-y-4">
                                <div>
                                    <label for="edit_nama_kelurahan"
                                        class="block text-sm font-medium text-gray-700">Nama Kelurahan</label>
                                    <input type="text" id="edit_nama_kelurahan" name="nama_kelurahan"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                </div>
                            </div>
                        </div>
...
<x-caution-modal id="delete-kelurahan-modal" title="Hapus Kelurahan"
    message="Apakah Anda yakin ingin menghapus kelurahan ini? Tindakan ini tidak dapat dibatalkan."
    confirmButtonId="confirm-delete-kelurahan-button" confirmText="Hapus" cancelText="Batal" />
...
<script>
    ...
    document.querySelectorAll('[id^="confirm-delete-"]').forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (dataIdToDelete) {
                fetch(`/kelurahan/${dataIdToDelete}`, {
                    method: "DELETE",
                    headers: {
    ...
    function openEditModal(id, nama) {
        ...
        document.getElementById('edit_kelurahan_id').value = id;
        document.getElementById('edit_nama_kelurahan').value = nama;

        // Dynamically set form action
        document.getElementById('edit-kelurahan-form').action = `/kelurahan/${id}`;

        // Show modal
        const modal = document.getElementById('edit-kelurahan-modal');
        modal.classList.remove('hidden');
        document.body.classList.add('overflow-hidden');
    }
    ...
    function openTambahModal() {
        const modal = document.getElementById('tambah-kelurahan-modal');
        modal.classList.remove('hidden');
        document.body.classList.add('overflow-hidden');
    }
    ...
    document.getElementById('tambah-kelurahan-form').addEventListener('submit', function (e) {
        e.preventDefault();
    ...
    document.getElementById('edit-kelurahan-form').addEventListener('submit', function (e) {
        e.preventDefault();
